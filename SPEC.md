...
- **FR-2 — Status query** (headline op b). As a coordinator, I ask "where are we" and get only what I asked for, never the raw store.
  ...
  - **AC-28:** `wg_status` given a `status` filter (and no node id) returns `{status, ids}` — the ids of all nodes currently in that state — for the "what's blocked/active now" query, without dumping any node bodies. (Best-practices pass.)
