...
- **Workgraph-is-truth** — one git-tracked state file is the single source of truth; status is an instant query, never a prose read.
...
- **FR-2 — Status query** (headline op b). As a coordinator, I ask "where are we" and get only what I asked for, never the raw store.
  ...
  - **AC-5:** `wg_status <id>` returns that node's `status`, `kind`, `gate.kind`, `updated_at`/`updated_by` provenance (AC-29), `last_verify` summary (exit code + timestamp), `signoff` (key absent when none), and — only when the node is a `parent` — the child rollup (counts by status). Asserted field-by-field against a parent fixture and a leaf fixture.
  - **AC-28:** `wg_status` given a `status` filter (and no node id) returns `{status, ids}` — the ids of all nodes currently in that state — for the "what's blocked/active now" query, without dumping any node bodies. The filter accepts **one or more** states comma-separated (e.g. `active,ready,blocked`, whitespace tolerated), unioning them; an unmatched value yields no ids (not an error). The `wg_mermaid`/CLI `--status` slice takes the same comma-separated form. (Best-practices pass; multi-status folded in later.)

  *Mermaid layout default:* `wg_mermaid`/`workgraph mermaid` default `direction` to **`auto`** — a slice with no in-slice edges (e.g. a multi-status query of unrelated nodes) renders **`LR`**, which stacks the independent nodes into a vertical column (terminal-friendly, since `TD` sprawls disconnected nodes horizontally); a connected slice renders `TD`. Explicit `TD`/`LR` always wins. (Folded post-alpha — terminal-rendering ergonomics.)
...
- **FR-5 — Gate-authorship governance** (the D-4 analog; the load-bearing adversarial clamp). The executor can never author/weaken its own gate or reach `done`.
  - **AC-10:** `wg_set_gate` or `wg_add_dep` on a node whose status is **not** `triage` is rejected (gates and edges are immutable once a node leaves `triage`); the store is unchanged. (Removing a *wrong* edge is the deliberate exception — the plan-surface `wg_remove_dep`, AC-31/D-16; you can never *add* a prerequisite to already-cleared work, only drop one.)
...
- **FR-6 — Terminal vocabulary** (`deferred`/`resolved` structurally distinct from `done`).
  ...
  - **AC-15:** Readiness counts only `done` and `resolved` as terminal-good. A node with a `deferred`, `blocked`, or `archived` **dependency** does **not** become `ready`; it is flagged `blocked` (prerequisite abandoned). This is the *dependency* axis and is deliberately stricter than the *parent-rollup* axis: an `archived` **dependency** still blocks (the dependent may genuinely still need its output — released only by an explicit `wg_remove_dep`, D-16), whereas an `archived` **child** does not block its parent's sign-off (out-of-scope, not owed — AC-20/D-17).
...
- **FR-8 — Node lifecycle, mutation integrity & rollup.**
  ...
  - **AC-20:** A `parent` is **membership only** (it creates no dependency edge; only `deps` order work). A parent node's status query reports its child rollup (counts by status); a `parent` that itself carries a `command`/`manual` gate cannot reach `done` while any child is **unsettled** — i.e. non-terminal-good **and not `archived`**. An `archived` child (explicitly removed from this epic's scope — e.g. the old children of a superseded-and-rebuilt epic) does **not** block the parent's sign-off; a `deferred` child (postponed but still owed) **does** (D-17). The archived child still appears in the rollup counts (it is queryable, just not blocking). A parent with **zero children** behaves as a normal leaf for its own gate.
  ...
  - **AC-24:** `wg_remove_node` is rejected unless the node is **not yet started** (`triage` or `ready`) **and** no other node lists it in `deps` or `parent`; removal never leaves a dangling reference. (`ready` is included because a no-dep node auto-advances `triage → ready`, so a mistakenly-added node would otherwise be unremovable; clear blocking edges first with `wg_remove_dep`. To retire a *started* or terminal node, use `wg_defer`/`wg_archive`, not removal.)
  - **AC-31:** `wg_remove_dep <id> <dep>` (plan surface) removes `dep` from node `<id>`'s `deps` and re-runs the readiness recompute, so a `blocked` dependent of an abandoned prerequisite returns to `ready` once the dead edge is gone. It is allowed only while the dependent is `triage`, `ready`, or `blocked` (an `active`/`awaiting-signoff` dependent's edges stay immutable); removing an edge the node doesn't have is rejected with a validation error; the execute surface cannot call it (AC-11). It is the plan-surface **inverse of `wg_add_dep`** — the asymmetry (you may remove but not add a dep after `triage`) is intentional (D-16).
...
- **FR-9 — Provenance & reconciliation** (workgraph-is-truth applied to the write path itself). Every mutation records who changed what and when, and prompts the caller to keep the surrounding prose in sync.
  - **AC-29:** Every successful state-changing mutation stamps the touched node(s) with `updated_at` (the tool's UTC clock) and `updated_by` — the caller-supplied `who` (a human handle, or an agent `role` / `role:node`), falling back to the calling surface (`plan`/`execute`) when omitted, so it is never empty. Surfaced in `wg_status <id>` (AC-5) and `wg_show`; persisted through a store round-trip. (D-15.)
...
Rules: `done` is reachable **only** via `wg_signoff` on the plan/operator surface (FR-5). `none`-gate nodes reach `resolved`, never `done`; `command`/`manual` nodes reach `done`, never `resolved`. A gated `parent` cannot reach `done` while any child is unsettled — non-terminal-good and not `archived` (an out-of-scope `archived` child doesn't block; a still-owed `deferred` child does — AC-20/D-17). The **server drives readiness** (AC-23) inside every write op — there is no executor-callable "recompute" tool, and a node never silently sits ready-but-not-flagged. `wg_remove_node` is constrained by AC-24 (not-yet-started + no dependents); `wg_remove_dep` drops an edge to release a `blocked` dependent of an abandoned prerequisite (AC-31). `wg_unblock` clears the manual block and re-enters the readiness recompute (AC-23), which deterministically routes the node to `ready` if its deps are terminal-good or leaves it `blocked` otherwise — no operator choice between `ready`/`active`.
...
- **C-5 — MCP tool contract.** Tools, grouped by surface (group membership is what the consumer's agent allowlist enforces — see Operational guardrails):
  ...
  - **Plan/operator:** `wg_ingest`, `wg_add_node`, `wg_set_gate`, `wg_add_dep`, `wg_remove_dep`, `wg_remove_node`, `wg_signoff`, `wg_resolve`, `wg_defer`, `wg_unblock`, `wg_archive`.
...
- milestone: M-1
- summary: `transition(Graph,…) -> Graph` — encode the transition table; enforce surface capability (X/P), gate/dep/parent-immutability-after-triage, the `done`-only-via-signoff clamp, the `deferred`↔`done` / `resolved`↔`done` exclusions, the readiness recompute (AC-23), the `wg_remove_node` constraint (AC-24) + `wg_remove_dep` edge removal (AC-31), the `wg_reverify` re-open (AC-25), and the manual-gate rationale requirement (AC-26). No load/save — the MCP server wraps store I/O around this.
...
- **D-2 — MCP-first; no CRUD CLI in MVP.** Minimal CLI only (`init`, `serve`, and the read-only `mermaid` projection added in the visualization pass — still no create/update/delete CLI; the agent's write surface is MCP). **Human sign-off is performed by the coordinator agent via the operator MCP tool, on the human's instruction**, after evidence is surfaced. *Rationale:* the consumer is an AI coordinator; sign-off needn't be a CLI. *Rejected:* CLI-first, both-surfaces-equally. *(Ratified — handoff Q2; safe only in combination with D-3, see R-1.)*
...
- **D-7 — `done` requires gate evidence (command exit 0 and/or manual sign-off) AND an operator-surface ack.** Executors have no transition into `done`. Gated parents reach `done` only when every child is settled — terminal-good or `archived` (out of scope); a still-owed `deferred` child blocks (D-17). *Rationale:* the only-a-lead-marks-done principle, made structural.
...
- **D-14 — Every state-changing mutation returns a `nudge` reminding the agent to reconcile the project's prose (status docs, work logs) with the workgraph; reinforced by server-level `instructions`.** The workgraph is the source of truth, but an agent that mutates it can leave the surrounding prose stale — so each mutation response carries a short `nudge` string keyed to the resulting status (terminal `done`/`resolved`/`deferred`/`archived` and `blocked` get sharper wording; a *failed* `wg_verify` settles nothing and so carries none), and the MCP server advertises `server_instructions()` at connect time stating the contract and that the nudge is actionable. *Rationale:* "workgraph-is-truth / no-drift" is only real if the prose is actually kept in sync; the nudge makes the reconciliation a default step at the exact moment drift is introduced, which is also where the tool's reason-for-being ("'deferred' is never mistaken for 'done'") bites hardest. The nudge is advisory (a prompt, not an enforced invariant) and ephemeral (response-only — never persisted to `graph.yaml`, preserving NFR-4 compactness). *Rejected:* a hard gate forcing a doc edit before the transition commits (couples the tool to the consumer's doc layout, which it deliberately doesn't know); nudging on reads (noise) or on no-op/failed transitions (nothing to reconcile).
- **D-15 — Every transition stamps per-node provenance (`updated_at` + `updated_by`).** Originally only `done` nodes carried who/at (via the `signoff` vouch); everything else leaned on D-5's git history, which is coarse (git's actor = the committer, time = commit time) and fragile (needs a commit per transition). So the service now stamps `updated_at` (the tool clock — `lifecycle` stays clockless) and `updated_by` on every mutation; `who` is a caller-supplied actor threaded through every mutating tool/CLI call, falling back to the calling surface when omitted so it is never empty (AC-29). *Rationale:* cheap, always-present provenance independent of commit discipline; `signoff` stays the distinct `done` governance vouch. *Rejected:* relying on git history alone (coarse/fragile); a mandatory `who` (too rigid — the surface fallback keeps it non-empty). *(Folded post-alpha.)*
- **D-16 — Dependency edges are removable (`wg_remove_dep`) and a not-yet-started node is deletable; abandoning a prerequisite is an explicit edge dissolution, not an auto-release.** Added `wg_remove_dep` (the plan-surface inverse of `wg_add_dep`) and relaxed `wg_remove_node` to `{triage, ready}` (AC-24/AC-31). *Why:* an operator agent hit a dead end — a deferred prerequisite blocked its dependents with no recourse (no remove-dep tool), and a mistakenly-added no-dep node had auto-advanced to `ready` and so was unremovable. The safe default stays: an abandoned (`deferred`/`archived`/`blocked`) dep **blocks** its dependents (AC-15/AC-23), because the dependent may genuinely still need it. The escape valve is an explicit, per-dependent operator decision to remove the edge, after which recompute releases the dependent. The add/remove asymmetry (AC-10 vs AC-31) holds because *adding* a prerequisite to cleared work would retroactively gate it (the self-gating hole D-3 closes), whereas *removing* a wrong/abandoned one is a loosening correction the operator legitimately owns — and it is plan-surface only, so a doer can't self-release. Removal is restricted to not-yet-started/blocked dependents so in-flight (`active`/`awaiting-signoff`) and terminal work is never reshaped. *Rejected:* auto-releasing dependents on defer/archive (conflates "X is abandoned" with "Y no longer needs X" — two separate judgments); a `wg_remove_dep` usable on in-flight nodes (would let prerequisites change mid-verification). *(Folded post-alpha — operator-reported gap.)*
- **D-17 — An `archived` child does not block its parent's sign-off; a `deferred` child does. The parent-rollup predicate is `terminal-good ∪ {archived}` (`ROLLUP_SETTLED`), deliberately wider than the dependency predicate `terminal-good` (AC-20).** *Why:* an operator superseded-and-rebuilt an epic — `archive`-ing its 15 first-build children and adding a fresh set under the same parent. Because the rollup reused the dependency predicate (`terminal-good = {done, resolved}`), the archived children blocked the parent's `wg_signoff` permanently: `archived` is terminal with no exit transition (D-6), and `wg_remove_node` only deletes `{triage, ready}` nodes (AC-24), so the children could be neither completed, un-archived, nor deleted — the epic was wedged. Root cause: one predicate was doing two semantically different jobs. *The distinction, now operational:* `archived` = explicitly removed from this epic's **scope** → not "owed," so the parent's `done` does not overclaim it → doesn't block; `deferred` = postponed but **still owed/in scope** → a parent claiming `done` over it would be exactly the "deferred mistaken for done" conflation this tool exists to prevent → still blocks (as do all non-terminal children). This preserves the core invariant: the only child state that newly stops blocking is an *explicit operator scope-removal*, and both `archive` and `signoff` are **plan-surface only** (a doer cannot archive its own incomplete children to fake a parent `done`) — the same trusted authority makes a recorded (git-provenanced) scope decision, squarely inside the governance model. The *dependency* axis is untouched: an `archived` **dependency** still blocks its dependents (AC-15), released only by an explicit `wg_remove_dep` (D-16) — abandoning a prerequisite and de-scoping a child are different judgments. *Rejected:* a first-class "supersede" relationship for rebuilt epics (YAGNI — archive-old + add-new under the same parent already models it once the rollup stops blocking on archived); letting `deferred` children also clear (would weaken the `deferred ≠ done` guarantee — the tool's reason for being); a transition out of `archived` / making archived children deletable (treats the symptom, not the conflated predicate, and reopens the "rewrite terminal work" surface AC-24 closes). *(Folded post-alpha — operator-reported gap: a superseded-and-rebuilt epic could not reach `done`.)*
...
- **Review gate before a unit is `done`:** all its `acceptance` IDs have a passing test, and the executor-surface restriction (R-1) is asserted (AC-11). Every alpha AC (AC-1…AC-27) is referenced by at least one work unit; the post-alpha AC-28…AC-30 (status filter, provenance, nudge) sit outside the original WU graph but are each covered by a test class here. Every AC and NFR (NFR-1…5) is covered by a test.
...
*Reconciliation nudge (D-14):* every state-changing mutation now returns a `nudge` field — a short reminder, keyed to the resulting status, to reconcile the project's prose (status docs, work logs, comments) with the workgraph. Terminal transitions carry the sharpest wording (`done`/`resolved` → "record it complete"; `deferred`/`archived` → "set aside, NOT completed — nothing should imply it shipped"; `blocked` → "note the blocker"); a *failed* `wg_verify` settles nothing and so returns no nudge; **reads never nudge**. Reinforced server-side: `build_server` now advertises `server_instructions()` (workgraph-is-truth + "the nudge is actionable") to the MCP client at connect time. The nudge is advisory and response-only — never persisted to `graph.yaml` (NFR-4). The live stdio test asserts both the instructions and a per-mutation nudge cross the real wire. 151 tests.

*Spec-reconciliation pass (fold the post-alpha features into the normative body, not just the changelogs):* added **FR-9** with **AC-29** (per-node provenance stamping, anchored by new **D-15**) and **AC-30** (the reconciliation nudge + `server_instructions()`, under D-14); added **AC-28** for the `wg_status` `status` filter; surfaced `updated_at`/`updated_by` in **AC-5**; updated **D-2** and **WU-7** to `init`/`serve`/read-only `mermaid`; wired AC-28…AC-30 into the test strategy (`test_provenance`/`test_nudge`/`test_live_mcp`/`test_render`) and noted they sit outside the original WU graph. No behaviour change — the spec now matches the code, not only the appended changelogs. 151 tests.

*Dependency-edge removal (D-16, operator-reported gap):* an operator agent had no way to dissolve a dependency — a deferred prerequisite blocked its dependents forever, and a mistakenly-added no-dep node auto-advanced to `ready` and so was unremovable. Added **`wg_remove_dep`** (plan surface, the inverse of `wg_add_dep`; allowed on a `triage`/`ready`/`blocked` dependent — recompute then releases a blocked dependent to `ready`; AC-31) and relaxed **`wg_remove_node`** to `{triage, ready}` (not-yet-started; AC-24). The abandon-blocks-dependents default (AC-15/AC-23) is unchanged — `wg_remove_dep` is the explicit, per-dependent release. Workflows: *abandon a planned task* = `wg_remove_dep` each dependent → `wg_defer`/`wg_archive` it; *delete a wrongly-added node* = `wg_remove_dep` its dependents → `wg_remove_node`. New tool added to the plan group + the executor `permissions.deny` list (governance). Discoverable over the MCP without the docs: `wg_remove_node`'s description names the remedy ("clear edges first with wg_remove_dep"), and the remove-with-dependents error names both the blocking dependents and the fix ("remove those dependency edges first, then retry"), so the ordering is recoverable from the tool surface and the error alone (actionable-errors). 162 tests.

*Archived children don't block a parent's sign-off (D-17, operator-reported gap):* superseding-and-rebuilding an epic — `archive`-ing its old children and adding a fresh set under the same parent — wedged the parent permanently: the rollup gate (AC-20) reused the *dependency* predicate `terminal-good = {done, resolved}`, so the archived children blocked `wg_signoff` with no recourse (`archived` is terminal with no exit, and `wg_remove_node` only deletes not-yet-started nodes). Split the predicates: parent rollup now uses **`ROLLUP_SETTLED = terminal-good ∪ {archived}`** (models.py), so an `archived` child (out of scope, not owed) no longer blocks the parent, while a `deferred` child (still owed) and every non-terminal child still do — preserving `deferred ≠ done`. The *dependency* axis is unchanged: an `archived` **dependency** still blocks its dependents (AC-15), released only by `wg_remove_dep` (D-16). Governance intact — `archive` and `signoff` are both plan-surface, so a doer can't archive its own incomplete children to fake a parent `done`. The archived child still shows in the rollup counts. The sign-off-rejected error now reads "a child is unsettled — every child must be terminal-good or archived (a deferred child is still owed and blocks)" so the remedy (archive the out-of-scope child, don't defer it) is recoverable from the error alone. 169 tests.
